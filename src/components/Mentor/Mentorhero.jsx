...
const MentorHero = () => {
    ...
    const FetchIndex = async () => {
        const base_url = "/api/mothership/mentor"
        const response = await axios.get(base_url)
        const result = response.data
        console.log(result.data[0].attributes.field_mentor_hero_caption.value)
        setIndex(result.data[0].attributes.field_mentor_hero_caption.value)
        setTitle(result.data[0].attributes.field_mentor_hero_title)
    ...
    if (typeof window !== 'undefined') {
        ...
        localStorage.setItem('mentorC', plainText);
        localStorage.setItem('mentorTitle', title);
